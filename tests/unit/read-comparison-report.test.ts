...
import { buildReadComparisonReportRow, createReadComparisonPhase } from '../../tests/load/read-comparison-report';
...
describe('read-only cache comparison reporting', () => {
    test('calculates phase statistics and speedup relative to raw throughput', () => {
        ...
        const phase = createReadComparisonPhase('warm', metrics.summarize(1_000), 'warm-digest', 2);
    ...
    test('renders the raw/cold/warm comparison row with the raw baseline at 1.00x', () => {
        const metrics = new BenchmarkMetrics();
        metrics.recordOperation('read', 12);
        const row = buildReadComparisonReportRow(
            createReadComparisonPhase('raw', metrics.summarize(1_000), 'raw-digest', 1),
        );
